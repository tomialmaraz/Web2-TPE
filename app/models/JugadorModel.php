...
class JugadorModel extends UserModel{
   ...
   function getJugadoresByClub($club){
        $query = $this->dataBase->prepare('SELECT jugadores.*, clubes.nombre AS nombre_club FROM jugadores INNER JOIN clubes ON jugadores.id_club = clubes.id_club WHERE clubes.id_club=?');
        $query->execute([$club]);
        $jugadores = $query->fetchAll(PDO::FETCH_OBJ);
        return $jugadores;
    }
    ...
    function getJugadoresByPosicion($posicion){
        $query = $this->dataBase->prepare('SELECT jugadores.*, clubes.nombre AS nombre_club FROM jugadores INNER JOIN clubes ON jugadores.id_club = clubes.id_club WHERE jugadores.posicion=?');
        $query->execute([$posicion]);
        $jugadores = $query->fetchAll(PDO::FETCH_OBJ);
        return $jugadores;
    }
